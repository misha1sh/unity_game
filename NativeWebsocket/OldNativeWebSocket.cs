...
namespace NativeWebSocket
{
   ...
   /// <summary>
    ...
    public class WebSocket : IWebSocket
    {
            ...
            if (!sending)
            {
               ...
               /* if (!Monitor.TryEnter(m_Socket, 1000))
                ...
                try
                {
                    ...
                    await m_Socket.SendAsync(buffer, messageType, true, CancellationToken.None);
                    // t.Wait();
                }
                finally
                {
        ...
        private MemoryStream receiveMS = new MemoryStream();
        private ArraySegment<byte> buffer = new ArraySegment<byte>(new byte[8192 * 10]);
        public Task Receive()
        {

            if (m_Socket.State != System.Net.WebSockets.WebSocketState.Open) return Task.CompletedTask;
            ...
            if (receiveTask != null)
            {
                if (receiveTask.IsCompleted)
                {
                    ...
                    receiveMS.Write(buffer.Array, buffer.Offset, result.Count);
                    ...
                    if (result.EndOfMessage)
                    {
                        ...
                        {
                            OnMessage?.Invoke(receiveMS.ToArray());
            ...
            {
                receiveTask = m_Socket.ReceiveAsync(buffer, CancellationToken.None);
